...
var ASM_CONSTS = [function() { const imports = { wasi_unstable: WASIPolyfill };         let file = document.getElementById('input').files[0];         let file_with_mime_type = file.slice(0, file.size, 'application/wasm');         let response = new Response(file_with_mime_type);         WebAssembly.instantiateStreaming(response, imports)         .then(obj => {             setInstance(obj.instance);             try {                 obj.instance.exports._start();             } catch (e) {                 if (e instanceof WASIExit) {                     handleWASIExit(e);                 } else {                     console.log(e);                 }             }         })         .catch(error => {             console.log('error! ' + error);         }); }];
...
// {{PRE_LIBRARY}}
  ...
  var _stderr=6016;var FS={root:null,mounts:[],devices:{},streams:[],nextInode:1,nameTable:null,currentPath:"/",initialized:false,ignorePermissions:true,trackingDelegate:{},tracking:{openFlags:{READ:1,WRITE:2}},ErrnoError:null,genericErrors:{},filesystems:null,syncFSRequests:0,handleFSError:function (e) {
